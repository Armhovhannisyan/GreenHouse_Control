...
const ManualOverridePage = (() => {
  ...
  function buildRelayRows(devices, relayModes) {
    ...
    (Array.isArray(devices) ? devices : []).forEach((d) => {
      const deviceName = d && d.name ? d.name : d && d.deviceid ? d.deviceid : 'Unknown';
      ...
      if (switches && switches.length) {
        switches.forEach((s, idx) => {
          ...
          const channelName =
            s && typeof s.name === 'string' && s.name.trim() ? s.name.trim() : '';
          rows.push({
            deviceName,
            channelName,
            multiChannel: switches.length > 1,
            id,
            channel,
            state: on ? 'On' : 'Off',
            online,
    ...
    return applyFriendlyRelayNames(rows);
  }

  function relayRowLabel(r) {
    const cn = String(r.channelName || '').trim();
    if (cn) return r.deviceName + ' · ' + cn;
    if (r.multiChannel) return r.deviceName + ' · ch ' + r.channel;
    return r.deviceName;
  }

  /** Display-only channel labels (eWeLink device id unchanged for API). */
  function applyFriendlyRelayNames(rows) {
    let w1Seq = 0;
    return rows.map(function (r) {
      if (String(r.deviceName || '').trim() !== 'W1_W2_L_MV1') return r;
      w1Seq += 1;
      if (w1Seq === 1) return Object.assign({}, r, { channelName: 'windows close' });
      if (w1Seq === 2) return Object.assign({}, r, { channelName: 'windows open' });
      return r;
    });
  }
  ...
  function renderRelayList(devices, relayModes, errorMessage) {
    ...
    const maxNameLen = rows.reduce((acc, r) => Math.max(acc, String(relayRowLabel(r) || '').length), 0);
    ...
    list.innerHTML = rows
      ...
      <div class="manual-override-row">
        <div class="manual-override-line">
          <span class="manual-override-name">${escapeHtml(relayRowLabel(r))}${r.online ? '' : ' (offline)'}</span>
