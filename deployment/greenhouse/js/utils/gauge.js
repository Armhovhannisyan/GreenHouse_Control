    ...
    let text =
      display != null && display !== ''
        ...
        : Number.isFinite(v)
          ...
          : '—';
    const unitEl =
      lblValue.parentElement && lblValue.parentElement.querySelector
        ? lblValue.parentElement.querySelector('.gauge-unit-inline')
        : null;
    const unit = unitEl && unitEl.textContent ? String(unitEl.textContent).trim() : '';
    if (unit && text !== '—' && text.endsWith(unit)) {
      text = text.slice(0, -unit.length).replace(/\s+$/, '');
    }
    lblValue.textContent = text;
  }
